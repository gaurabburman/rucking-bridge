...
module.exports = async (req, res) => {
    // 🛡️ 1. CORS SHIELD (Allowing Secure Retrieval)
    ...
    try {
        ...
        // 🛡️ 3. PRIVATE KEY MUTATION (Vercel Fix)
        let privateKey = process.env.FIREBASE_PRIVATE_KEY.replace(/\\n/g, '\n');
        if (privateKey.startsWith('"') && privateKey.endsWith('"')) {
            privateKey = privateKey.slice(1, -1);
        }

        // 🛡️ 4. INITIALIZE FIREBASE (Only once)
        ...
        // 🛡️ 7. FETCH FULL SPECTRUM DATA (Every Single Metric)
        // Hum operator_uid match karenge aur latest entries uthayenge
        const logs = await db.collection('personal_logs')
            .find({ operator_uid: uid })
            .sort({ timestamp: -1 }) // Newest first
            .limit(15) // Strictly last 15 tactical logs
            .toArray();

        await client.close();

        // 🛡️ 8. RETURN FULL PACKET
        return res.status(200).json({
            success: true,
            message: 'Full Tactical Intel Retrieved Successfully!',
            count: logs.length,
            data: logs // Isme ab har ek detail (BMI, Slope, terrain, etc.) shamil hai
        });
